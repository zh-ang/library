...
class Zend_Uri_Http extends Zend_Uri
{
    ...
    /**
     * Validates a domain hostname
     *
     * @param  string $value Value to check against
     * @return boolean True when $value is a valid domain
     *                 False otherwise
     */
    protected function _validateDomain($value)
    {
        ...
        if ((count($domainParts) > 1) && (strlen($value) >= 4) && (strlen($value) <= 254)) {
            ...
            foreach ($domainParts as $domainPart) {
                ...
                if (strpos($domainPart, 'xn--') === 0) {
                    // TODO $domainPart = $this->decodePunycode(substr($domainPart, 4));
                    return false;
                }

                if (($domainPart[0] === '-') || ($domainPart[strlen($domainPart) - 1] === '-')) {
                    return false;
                }

                if (substr($domainPart, 2, 2) === '--') {
                    return false;
                }

                $status = @preg_match('/^[a-z0-9\x2d]{1,63}$/i', $domainPart);
                if ($status == 0) {
                    return false;
                }

                if (iconv_strlen($domainPart, 'UTF-8') > 63) {
                    return false;
                }
            }
        }
    ...
    public function validateHost($host = null)
    {
        ...
        if (preg_match('/^[0-9a-f:.]*$/i', $host) && ($this->_validateIPv4($host) || $this->_validateIPv6($host))) {
            return true;
        }
